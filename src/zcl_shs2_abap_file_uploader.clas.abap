CLASS zcl_shs2_abap_file_uploader DEFINITION
  PUBLIC
  CREATE PUBLIC .
...
CLASS ZCL_SHS2_ABAP_FILE_UPLOADER IMPLEMENTATION.
...
* Load the data to the table via dynamic internal table
        ...
        TRY.
          ...
          CATCH cx_sy_create_data_error INTO DATA(cd_exception).
            response->set_status( i_code = if_web_http_status=>bad_request
                                 i_reason = cd_exception->get_longtext(  ) ).
            response->set_text( cd_exception->get_longtext(  )  ).
            RETURN.
        ENDTRY.
        ...
        LOOP AT <table_structure> ASSIGNING FIELD-SYMBOL(<table_record>).
        TRY.
*            INSERT (tablename) FROM TABLE @<table_structure>.
            MODIFY (tablename) FROM @<table_record>.
            IF sy-subrc = 0.
          ...
          CATCH cx_sy_open_sql_db INTO DATA(db_exception).
            response->set_status( i_code = if_web_http_status=>bad_request
                                 i_reason = db_exception->get_longtext(  ) ).
            response->set_text( db_exception->get_longtext(  )  ).
            "RETURN.
        ENDTRY.
        ENDLOOP.
